...
const SunIcon = ({ compact = false }) => (
  <svg
    ...
    style={compact ? undefined : { marginRight: '0.4rem' }}
    aria-hidden="true"
  >
...
// `compact` renders the icon-only circular variant used by the floating
// header; without it the button keeps its original pill + label look so any
...
const ThemeButton = ({ compact = false }) => {
  ...
  if (compact) {
    return (
      ...
      >
        {isDark ? <SunIcon compact /> : <MoonIcon compact />}
      </button>
    );
  }
